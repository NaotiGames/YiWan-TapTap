...
struct FTapHttpBuilder_Achievement : public FTapHttpBuilder
{
	FTapHttpBuilder_Achievement(const FString& InSessionToken, const FString& Verb, const TCHAR* Path, const TMap<FString, FString>& Query = {})
		: FTapHttpBuilder(Verb, FTUConfig::Get()->RegionType == ERegionType::CN ?  TUDebuger::GetReplacedHost(TEXT(ACHIEVEMENT_HOST_CN)) : TUDebuger::GetReplacedHost(TEXT(ACHIEVEMENT_HOST_IO)), Path, Query)
	{
		ContentType_Json();
		ClientID();
		LCSign();
